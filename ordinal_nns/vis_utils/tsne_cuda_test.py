...
from data_select_utils import select_test_dataset, select_dataset
...
def main(args):
    ...
    vec_data, tr_labels = select_dataset(args.dataset_name, testing=False)
    test_data, labels = select_test_dataset(args.dataset_name, testing=False)

    # compute the number of data samples
    n = vec_data.shape[0]
    # Input data to the network
    digits = int(math.ceil(math.log2(n)))
    bin_array_full = data_utils.get_binary_array(vec_data.shape[0], digits)  # Binary representation of the data
    bin_array = bin_array_full[0:test_data.shape[0], :]
    ...
    for i, c, label in zip(target_ids, colors, target_names):
        plt.scatter(embedded_x[labels == i, 0], embedded_x[labels == i, 1], s=4, c=c, label=label, alpha=0.9)
    plt.legend()
    # plt.show()

    os.makedirs('tsne_test', mode=0o777, exist_ok=True)
    embeddings_with_labels = {'embeddings': embedded_x, 'labels':labels}
    np.save(os.path.join('tsne_test', result_name + '.npy'), embeddings_with_labels)
    f.savefig(os.path.join('tsne_test', result_name + '.eps'), format='eps')
