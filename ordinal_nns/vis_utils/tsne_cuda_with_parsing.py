...
def main(args):
    ...
    pipeline_specs = baseline_model_parser(args.model_path)
    # fetch the device
    ...
    model_dict = torch.load(args.model_path)['model_state_dict']

    key_word = list(model_dict.keys())[0].split('.')[0]
    print(key_word)

    if key_word == 'module' and torch.cuda.device_count() > 1:
    ...
    data_bin = torch.Tensor(bin_array).to(device, non_blocking=True)
