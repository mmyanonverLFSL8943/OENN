...
def parse_args():
    ...
    parser.add_argument('-d', '--dataset_name', type=str, default='uniform', required=False,
    ...
    parser.add_argument('-minD', '--min_dim', type=int, default=2, required=False,
                        help='Minimum embedding dimension')
    parser.add_argument('-maxD', '--max_dim', type=int, default=25, required=False,
    ...
    parser.add_argument('-rs', '--range_scale', type=int, default= 1000, help= 'the scale multiplied by np.arange(1,11)')
...
def main(args):
    ...
    vec_data_total, labels = select_dataset(args.dataset_name, n = 100000, input_dim = 2)
    ...
    n_range = args.range_scale * np.array(range(1, 11))
    n_range = np.concatenate((n_range,np.array([20000,50000, 100000])))
    print(n_range)
    train_errors = list()
    test_errors = list()
    times = list()
    ...
    for n in n_range:
        ...
        if n < 20000:
            ...
            begin = time.time()
            res = os.system('Rscript --no-save --no-restore --verbose runLOE.R')
            print('LOE res=',res)
        ...
        embedding_OENN, time_OENN= training_routine.create_and_train_triplet_network_withtime(experiment_name=experiment_name,
...
if __name__ == "__main__":
    # main(parse_args())
    datasetname = 'MNIST'
    comparison_files = np.load('comparison-results/OENN-TSTE.npz')

    times = np.array(comparison_files['times'])
    n_range = 100 * np.array(range(1, 11))
    fig, axes = plt.subplots(1,1,figsize = (4,3.5))
    # print(times.shape)
    axes.plot(n_range, times[:,0], marker = 'x')
    axes.plot(n_range, times[:, 1], marker='o')
    axes.plot(n_range, times[:, 2], marker='+')

    axes.set_xlabel('Number of items (n)',fontsize = 13)
    axes.set_ylabel('Training time (seconds)',fontsize = 13)
    axes.legend(['OENN','TSTE','LOE'])
    axes.set_title(str(datasetname) + '(dim = 50)')
    plt.tight_layout()

    fig.savefig('comparison-figs/'+datasetname+'-time-OENN-TSTE-LOE.eps')

    train_error = np.array(comparison_files['train_errors'])
    fig, axes = plt.subplots(1, 1, figsize=(4, 3.5))

    axes.plot(n_range, train_error[:, 0], marker='x')
    axes.plot(n_range, train_error[:, 1], marker='o')
    axes.plot(n_range, train_error[:, 2], marker='+')
    axes.set_xlabel('Number of items (n)',fontsize = 13)

    axes.set_ylabel('Training error',fontsize = 13)
    axes.legend(['OENN', 'TSTE', 'LOE'])
    axes.set_title(str(datasetname) + '(dim = 50)')
    axes.set_ylim([0, 0.25])
    plt.tight_layout()

    fig.savefig('comparison-figs/'+datasetname+'-train-OENN-TSTE-LOE.eps')

    test_error = np.array(comparison_files['test_errors'])
    fig, axes = plt.subplots(1, 1, figsize=(4, 3.5))

    axes.plot(n_range, test_error[:, 0], marker='x')
    axes.plot(n_range, test_error[:, 1], marker='o')
    axes.plot(n_range, test_error[:, 2], marker='+')
    axes.set_xlabel('Number of items (n)',fontsize = 13)
    axes.set_ylabel('Test error',fontsize = 13)
    axes.legend(['OENN', 'TSTE', 'LOE'])
    axes.set_title(str(datasetname) + '(dim = 50)')
    axes.set_ylim([0, 0.25])
    plt.tight_layout()
    fig.savefig('comparison-figs/'+datasetname+'-test-OENN-TSTE-LOE.eps')
    plt.show()
